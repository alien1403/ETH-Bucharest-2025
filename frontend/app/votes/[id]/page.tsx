...
import { encryptVote } from '@/lib/rust-api';
...
const MOCK_VOTES = [
	{
		...
		longDescription:
			'This proposal aims to upgrade our protocol to version 2.0, introducing several security improvements and optimizations. The upgrade will include:\n\n- Enhanced encryption algorithms\n- Improved transaction throughput\n- Better gas efficiency\n- New governance features',
		status: 'active',
		...
		options: [
			{
				id: '1',
				label: 'Yes',
				description: 'Approve the protocol upgrade',
			},
			{
				id: '2',
				label: 'No',
				description: 'Reject the protocol upgrade',
			},
			{
				id: '3',
				label: 'Abstain',
...
export default function VoteDetailPage() {
	...
	const handleVote = async () => {
		...
		try {
			...
			// Get the public key from the vote data
			if (!vote.publicKey) {
				throw new Error('Public key not found for this vote');
			}

			// Convert the public key from number array to Uint8Array
			const publicKey = new Uint8Array(vote.publicKey);

			// Encrypt the vote using the Rust API
			const encryptedVote = await encryptVote(
				publicKey,
				optionIndex + 1, // Convert to 1-based index for the Rust API
				vote.options.length
			);

			// Convert the encrypted vote to a hex string for the contract
			const encryptedVoteHex = bytesToHex(encryptedVote);

			// Call the contract to submit the vote
			await submitVote({
				campaignId: BigInt(voteId),
				encryptedVote: [encryptedVoteHex], // The contract expects an array of bytes
			});
